...
Enforces Rust-style TypeScript: bans optionals, booleans, `null`, `undefined`, `any`, `unknown`. Forces discriminated unions everywhere.

Ships as **two artifacts**:

- **Claude Code plugin**: PreToolUse `Write`/`Edit` hooks that block bad TypeScript before it lands on disk, plus skills and an agent that wire ESLint into your project.
- **ESLint plugin** ([`eslint-plugin-perfect-typescripter`](./eslint-plugin)): four cross-file rules the per-file hook can't see (phantom type params, duplicate envelopes, shared variant literals, prefix drift).
...
Clone this repo to a local marketplace directory and install from it:

```bash
git clone https://github.com/ChequePickerUpper/perfect-typescripter.git ~/perfect-typescripter

# Tell Claude Code about the parent directory as a marketplace,
...
Optional project config (`.claude/ai-lab/perfect-typescripter/config.json` in your project root):
...
The cross-file rules ship as a sibling npm package. After the first GitHub release, install via:

```bash
npm install --save-dev eslint-plugin-perfect-typescripter
```

Or wire it into a project automatically from inside Claude Code with `/setup-eslint`, which detects the package manager, ESLint config style, and pre-commit setup, then scaffolds the whole integration.

## Updating

```bash
cd ~/perfect-typescripter && git pull
claude plugin marketplace update perfect-typescripter
claude plugin update perfect-typescripter@perfect-typescripter
```

## Custom Detectors

Edit `hooks/perfect_typescripter_guard.py` to add project-specific rules:

```python
def detect_custom_anti_pattern(code: str) -> list[str]:
    violations = []
    
    # Example: Ban Array.includes for enums
    if "includes(" in code:
        violations.append("Use discriminated unions instead of includes()")
